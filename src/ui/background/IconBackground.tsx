...
const IconBackground: React.FC<IconBackgroundProps> = ({icons, iconSize = 24, iconOpacity = 0.1,}) => {
    const storedColor = Cookies.get('bgColor');
    const [customColor,] = useState<string>(storedColor || '#14151B');
    console.log('custom bg color', customColor);

    function getRandomPosition(minOffset = 0, maxOffset = 100): number {
        return Math.floor(Math.random() * (maxOffset - minOffset + 1)) + minOffset;
    }

    console.log("className:", `fixed inset-0 -z-10 overflow-hidden ${customColor && customColor != '' ? `bg-[${customColor}]` : 'dark:bg-[#14151B]'}`);
